...
# 增補段：建議加入的「防護與應對」清單（可直接放在章末）

* 技術層級

  1. 啟用 SPF、DKIM 與 DMARC（採 `p=reject` 策略，並監控報表）
  2. 郵件閘道：URL 換址掃描、附件沙箱、垃圾郵件/釣魚實時阻擋
  3. 電話安全：對敏感指令實施 callback policy、雙重確認或主管簽核
  4. 網站防護：強制 HTTPS、監控 DNS 變更、使用 HSTS 與 DNSSEC（可行時）
* 人員與流程

  1. 定期釣魚演習（phishing simulation）＋針對性回饋
  2. 事件回應 SOP：如何封鎖攻擊源、撤銷風險帳號、通知受影響單位
  3. 最小權限與多因素 authentication（MFA）
* 管理面

  1. 針對高風險職位（財務、HR、IT 高層）做加強訓練與流程限制
  2. 建立「快速驗證管道」（例如公司內部專線或單一 portal）以核實異常要求

# 技術控制（可直接放入章節）

* 電子郵件驗證：**SPF、DKIM、DMARC**（強制 reject / quarantine 原則，並監控報告）。
* 網域知識：設定 **MTA-STS / TLS-RPT** 以確保傳輸加密與問題通知。
* 郵件閘道：URL rewrite + 點擊時間檢查（先把短網址改為可檢測代理、在點開前檢查分類）。
* 附件沙箱：在用戶端執行前於隔離環境檢測 Office/可執行檔的惡意行為。
* 網站信任指示：部署 **BIMI**（品牌指紋圖標）與 HTTPS 強制，減少 UI 欺騙成功率。
* 身份與存取：**強制多因素認證（MFA）**、最小權限、臨時權限升級流程（兩人核准）。
* 端點檢測：EDR + 網路分段 + NAC，防止從單一受害帳戶橫向移動。
* USB / 可移動媒體：disable autorun、阻擋未授權 USB、建立檢測和回收流程（實體標籤）。
* 電話防護：設定企業內線驗證碼（call-back root）；對關鍵轉帳設定 out‑of‑band 確認（例如用已知手機號碼回撥）。

# 人員 / 教育（實務可落地）

* 微學習（microlearning）：每週 3–5 分鐘短影片或一張卡片提示重點（不要用長講座取代）。
* 模擬釣魚：分層模擬（高階主管 vs 一般員工 vs 財務），並以教學回饋（非懲罰）。
* 角色化訓練：給不同部門（HR、財務、IT、客服）專門範例與檢查清單。
* 電話驗證訓練：教社員工如何以已知聯絡方式「回撥驗證」，以及驗證必要的識別資訊（不在電話中透露敏感資訊）。
* 心理層面教育：教導常見心理誘因（權威、急迫性、恐懼、好奇）與如何冷卻（暫停、雙重確認）。
